...
public interface LineageDao {
     ...
     * or produced by the input jobIds. This returns a single layer from the BFS
     * using datasets as edges. Jobs that have no input or output datasets will have
     * no results. Jobs that have no upstream producers or downstream consumers will
     * have the original jobIds returned.
     *
     * @param jobIds
     * @return
     */
    ...
    @SqlQuery("""
                WITH RECURSIVE upstream(job_uuid, job_symlink_target_uuid, depth) AS (
                    -- Base case: direct inputs
                    SELECT DISTINCT
                        j2.job_uuid,
                        j2.job_symlink_target_uuid,
                        1 as depth
                    FROM job_versions_io_mapping j1
                    JOIN job_versions_io_mapping j2 ON
                        j2.dataset_uuid = ANY(
                            SELECT dataset_uuid
                    ...
                    WHERE j1.is_current_job_version = TRUE
                    AND j2.is_current_job_version = TRUE
                    AND j2.io_type = 'OUTPUT'

                    UNION ALL

                    -- Recursive case: get next level of inputs
                    SELECT DISTINCT
                        j2.job_uuid,
                        j2.job_symlink_target_uuid,
                        u.depth + 1
                    FROM upstream u
                    JOIN job_versions_io_mapping j1 ON
                        j1.job_uuid = u.job_uuid
                    JOIN job_versions_io_mapping j2 ON
                        j2.dataset_uuid = j1.dataset_uuid
                    WHERE j1.is_current_job_version = TRUE
                    AND j2.is_current_job_version = TRUE
                    AND j1.io_type = 'INPUT'
                    AND j2.io_type = 'OUTPUT'
                    AND u.depth < :depth
                )
                SELECT DISTINCT ON (j.uuid) j.*,
                    array_agg(DISTINCT CASE WHEN m.io_type = 'INPUT' THEN m.dataset_uuid END) AS input_uuids,
                    array_agg(DISTINCT CASE WHEN m.io_type = 'OUTPUT' THEN m.dataset_uuid END) AS output_uuids
                FROM upstream u
                INNER JOIN jobs_view j ON (j.uuid = u.job_uuid OR j.uuid = u.job_symlink_target_uuid)
                LEFT JOIN job_versions_io_mapping m ON m.job_uuid = j.uuid AND m.is_current_job_version = TRUE
                GROUP BY j.uuid, j.name, j.namespace, j.created_at, j.updated_at
            """)
    Set<JobData> getUpstreamLineage(@Bind("jobId") UUID jobId, @Bind("depth") int depth);

    @SqlQuery("""
                WITH RECURSIVE downstream(job_uuid, job_symlink_target_uuid, depth) AS (
                    -- Base case: direct outputs
                    SELECT DISTINCT
                        j2.job_uuid,
                        j2.job_symlink_target_uuid,
                        1 as depth
                    ...
                    WHERE j1.is_current_job_version = TRUE
                    AND j2.is_current_job_version = TRUE
                    AND j2.io_type = 'INPUT'

                    UNION ALL

                    -- Recursive case: get next level of outputs
                    SELECT DISTINCT
                        j2.job_uuid,
                        j2.job_symlink_target_uuid,
                        d.depth + 1
                    FROM downstream d
                    JOIN job_versions_io_mapping j1 ON
                        j1.job_uuid = d.job_uuid
                    JOIN job_versions_io_mapping j2 ON
                        j2.dataset_uuid = j1.dataset_uuid
                    WHERE j1.is_current_job_version = TRUE
                    AND j2.is_current_job_version = TRUE
                    AND j1.io_type = 'OUTPUT'
                    AND j2.io_type = 'INPUT'
                    AND d.depth < :depth
                )
                SELECT DISTINCT ON (j.uuid) j.*,
                    ...
                    array_agg(DISTINCT CASE WHEN m.io_type = 'OUTPUT' THEN m.dataset_uuid END) AS output_uuids
                FROM downstream d
                INNER JOIN jobs_view j ON (j.uuid = d.job_uuid OR j.uuid = d.job_symlink_target_uuid)
                LEFT JOIN job_versions_io_mapping m ON m.job_uuid = j.uuid AND m.is_current_job_version = TRUE
                GROUP BY j.uuid, j.name, j.namespace, j.created_at, j.updated_at
            """)
    Set<JobData> getDownstreamLineage(@Bind("jobId") UUID jobId, @Bind("depth") int depth);

    /**
     * Fetch the direct lineage for a specific job or dataset - only immediate
     * connections without traversing the entire lineage graph.
     *
     * @param nodeId The ID of the node to get direct lineage for
     * @return A set of JobData objects representing jobs directly connected to the
     *         specified node
     */
    @SqlQuery("""
            WITH job_io AS (
                -- Get input/output information for all jobs
                SELECT
                    io.job_uuid AS job_uuid,
                    io.job_symlink_target_uuid AS job_symlink_target_uuid,
                    ARRAY_AGG(DISTINCT io.dataset_uuid) FILTER (WHERE io.io_type='INPUT') AS inputs,
            ...
            dataset_connections AS (
                SELECT DISTINCT
                    ...
                    io.outputs
                FROM jobs_view j
                INNER JOIN job_io io ON j.uuid = io.job_uuid
                WHERE
                    (:nodeType = 'DATASET' AND (:datasetUuid IS NULL OR
                        :datasetUuid = ANY(io.inputs) OR :datasetUuid = ANY(io.outputs)))
            ),
            job_connections AS (
                SELECT DISTINCT
                    j.uuid AS job_uuid,
                    ...
                    io.outputs
                FROM jobs_view j
                LEFT JOIN job_io io ON j.uuid = io.job_uuid
                WHERE (:nodeType = 'JOB' AND (:jobUuid IS NULL OR j.uuid = :jobUuid))
            ),
            combined_connections AS (
                SELECT * FROM dataset_connections
                UNION
                SELECT * FROM job_connections
            )
            SELECT DISTINCT ON (j.uuid) j.*,
                cc.inputs AS input_uuids,
                cc.outputs AS output_uuids
            FROM combined_connections cc
            INNER JOIN jobs_view j ON (j.uuid = cc.job_uuid OR j.uuid = cc.job_symlink_target_uuid)
            ORDER BY j.uuid, j.updated_at DESC
            """)
    Set<JobData> getDirectLineage(@Bind("nodeType") String nodeType, @Bind("jobUuid") UUID jobUuid,
